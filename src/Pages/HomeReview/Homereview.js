...
const Homereview = () => {
    ...
    useEffect(() => {
        fetch("https://cryptic-chamber-94490.herokuapp.com/userreviews")
            .then(res => res.json())
            ...
            <div className='container row mx-auto' style={{ justifyContent: "center" }}>
                {
                    review.map(singleReview => <Card
                        ...
                        <Card.Body>
                            <Card.Title className='fw-bolder fs-4 text-center'>{singleReview.name} says ABOUT <br /> <span >{singleReview.productName}</span></Card.Title>
